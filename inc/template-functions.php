 ...
 * @return array
 */
function Dudley_body_classes( $classes ) {
...
add_filter( 'body_class', 'Dudley_body_classes' );
 ...
 * @return array
 */
function Dudley_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'Dudley_post_classes', 10, 3 );
 ...
 * @return void
 */
function Dudley_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'Dudley_pingback_header' );
 ...
 * @return void
 */
function Dudley_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'Dudley_supports_js' );
 ...
 * @return array
 */
function Dudley_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'Dudley_comment_form_defaults' );
 ...
 * @return bool
 */
function Dudley_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'Dudley_can_show_post_thumbnail',
 ...
 * @return int
 */
function Dudley_get_avatar_size() {
	return 60;
}
 ...
 * @since Dudley 1.0
 */
function Dudley_continue_reading_text() {
 ...
 * @since Dudley 1.0
 */
function Dudley_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . Dudley_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'Dudley_continue_reading_link_excerpt' );
 ...
 * @since Dudley 1.0
 */
function Dudley_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . Dudley_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'Dudley_continue_reading_link' );

if ( ! function_exists( 'Dudley_post_title' ) ) {
	/**
 ...
 * @return string
 */
function Dudley_get_icon_svg( $group, $icon, $size = 24 ) {
	return Dudley_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function Dudley_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? Dudley_get_icon_svg( 'ui', 'arrow_right' ) : Dudley_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? Dudley_get_icon_svg( 'ui', 'arrow_left' ) : Dudley_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'Dudley_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function Dudley_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'Dudley_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'Dudley_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'Dudley_generate_css' ) ) {
	...
	return Dudley_generate_css( // @phpstan-ignore-line.
 ...
 */
function Dudley_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since Dudley 1.0
 * @since Dudley 1.4 Corrected parameter name for `$output`,
 ...
 */
function Dudley_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
...
add_filter( 'the_password_form', 'Dudley_password_form', 10, 2 );
 ...
 * @return array
 */
function Dudley_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'Dudley_get_attachment_image_attributes', 10, 3 );
