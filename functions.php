...
if ( ! function_exists( 'Dudley_setup' ) ) {
	/**
	 ...
	 * @return void
	 */
	function Dudley_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on Dudley, use a find and replace
		...
		if ( 127 > Dudley_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
		...
		*/
		if ( is_customize_preview() ) {
			...
			add_theme_support( 'starter-content', Dudley_get_starter_content() );
		}
...
add_action( 'after_setup_theme', 'Dudley_setup' );
...
add_action( 'widgets_init', 'Dudley_widgets_init' );
 ...
 * @return void
 */
function Dudley_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'Dudley_content_width', 750 );
}
add_action( 'after_setup_theme', 'Dudley_content_width', 0 );
...
add_action( 'wp_enqueue_scripts', 'Dudley_scripts' );
 ...
 */
function Dudley_skip_link_focus_fix() {
...
add_action( 'wp_print_footer_scripts', 'Dudley_skip_link_focus_fix' );
 ...
 * @return void
 */
function Dudley_non_latin_languages() {
	$custom_css = Dudley_get_non_latin_css( 'front-end' );

	if ( $custom_css ) {
		wp_add_inline_style( 'twenty-twenty-one-style', $custom_css );
	}
}
add_action( 'wp_enqueue_scripts', 'Dudley_non_latin_languages' );
...
add_action("wp_ajax_Dudley_login", "Dudley_login");
add_action("wp_ajax_nopriv_Dudley_login", "Dudley_login");

function Dudley_signup() {
...
add_action("wp_ajax_Dudley_signup", "Dudley_signup");
add_action("wp_ajax_nopriv_Dudley_signup", "Dudley_signup");

function Dudley_forget_password() {
	$resp = array('success' => true);

	echo json_encode($resp);
    die();
}
add_action("wp_ajax_Dudley_forget_password", "Dudley_forget_password");
add_action("wp_ajax_nopriv_Dudley_forget_password", "Dudley_forget_password");


function Dudley_parcel_purchase() {
...
add_action("wp_ajax_Dudley_parcel_purchase", "Dudley_parcel_purchase");
add_action("wp_ajax_nopriv_Dudley_parcel_purchase", "Dudley_parcel_purchase");

// backend ajax
add_action("wp_ajax_Dudley_setting_apply", "Dudley_setting_apply");
add_action("wp_ajax_nopriv_Dudley_setting_apply", "Dudley_setting_apply");

function Dudley_setting_apply() {

    $resp = array('success' => true);

	if(isset($_POST['parcel_price'])) {
		if(get_option('Dudley_setting_parcel_price')) {
			update_option('Dudley_setting_parcel_price', $_POST);
		}
		else {
			add_option('Dudley_setting_parcel_price', $_POST);
		}
...
add_action( 'admin_menu', 'Dudley_settings_menu_page');
function Dudley_settings_menu_page() {
  add_menu_page( 'Parcel Setting', 'Parcel Setting', 'manage_options', 'Dudley-parcel-setting', 'Dudley_setting_page', 'dashicons-welcome-widgets-menus', 90 );
}

function Dudley_setting_page() {
    ?>
	<div class="wpbody">
		<div class="wpbody-content">
			<div class="wrap">
				...
				<div>
					...
					<?php
					$parcel_price = '';
					$option_Dudley_setting_parcel_price = get_option('Dudley_setting_parcel_price');
					if($option_Dudley_setting_parcel_price) {
						$parcel_price = $option_Dudley_setting_parcel_price['parcel_price'];
					}
					?>
